...
export interface EnvQueryParams {
    ...
    device_id?: number;     // 장치 ID (21, 22, 23 등)
}
...
export interface EnvQueryResponse {
    ...
    data: EnvDataPoint[];          // 실제 측정 데이터
    stats: EnvStats;               // 통계 정보
...
export async function fetchEnvQuery(params: EnvQueryParams = {}): Promise<EnvQueryResponse> {
    ...
    try {
        ...
        // 기본값으로 안전한 응답 구조 생성
        const response: EnvQueryResponse = {
 ...
 */
export async function fetchEnvHistory(deviceId: number, preset: string) {
    return fetchEnvQuery({
        device_id: deviceId,
        preset,
        max_points: 2000
