...
export async function fetchRealtime(deviceId) {
    const json = await fetchJSON(`/data/modbus/realtime?device_id=${deviceId}`);
...
export async function fetchModbusQuery(params) {
    ...
    if (params.series)
        q.set("series", params.series.join(","));
    if (params.preset)
    ...
    const json = await fetchJSON(`/data/modbus/query?${q.toString()}`);
    return {
...
export async function fetchTodayEnergy(deviceId) {
    const json = await fetchJSON(`/data/modbus/today?device_id=${deviceId}`);
