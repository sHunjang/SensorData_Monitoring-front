...
export default function CsvDownloader({ apiPath, extraParams = {}, filePrefix = 'data', defaultRangeHours = 1, }) {
    const now = new Date();
    ...
    const [startLocal, setStartLocal] = useState(fmtLocal(defaultStart));
    const [endLocal, setEndLocal] = useState(fmtLocal(defaultEnd));
    ...
    return (_jsxs("div", { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }, children: [_jsxs("label", { style: { fontSize: 12 }, children: ["\uC2DC\uC791", _jsx("input", { type: "datetime-local", value: startLocal, onChange: (e) => setStartLocal(e.target.value), style: { marginLeft: 6 } })] }), _jsxs("label", { style: { fontSize: 12 }, children: ["\uC885\uB8CC", _jsx("input", { type: "datetime-local", value: endLocal, onChange: (e) => setEndLocal(e.target.value), style: { marginLeft: 6 } })] }), _jsx("button", { onClick: handleDownload, disabled: loading, className: "button", children: loading ? '다운로드 준비중...' : 'CSV 다운로드' }), error ? _jsx("div", { style: { color: 'crimson', fontSize: 12 }, children: error }) : null] }));
}
