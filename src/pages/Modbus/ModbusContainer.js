...
export default function ModbusContainer() {
    ...
    const [column, setColumn] = useState('active_power');
    const [zoomLevel, setZoomLevel] = useState(2); // 🔧 ZoomLevel 타입 사용
    ...
    return (_jsx(ModbusPresenter, { deviceId: deviceId, setDeviceId: setDeviceId, deviceOptions: deviceOptions, column: column, setColumn: setColumn, zoomLevel: zoomLevel, zoomLabel: currentZoomConfig.label, onZoomIn: handleZoomIn, onZoomOut: handleZoomOut, canZoomIn: zoomLevel > 0, canZoomOut: zoomLevel < 5, onDataPointClick: handleDataPointClick, onManualRefresh: handleManualRefresh, data: data, stats: stats, loading: loading, error: error, logs: logs, peakLimits: peakLimits, setPeakLimits: setPeakLimits }));
}
