...
function extractFromRealtime(raw, candidates) {
    if (raw == null)
        return null;
    // 1) metrics 객체 우선
    const metrics = raw.metrics ?? raw.metrics ?? raw.metric ?? null;
...
export default function HomeContainer() {
    ...
    // 원시 응답(디버그/추가 처리용)
    const [rawModbus, setRawModbus] = useState(null);
    const [rawEnvLast, setRawEnvLast] = useState(null);
    const [rawSolarLast, setRawSolarLast] = useState(null);
    // 에러 상태
    ...
    return (_jsx(HomePresenter, { power: power, todayKwh: todayKwh, temperature: temperature, humidity: humidity, solar: solar, powerError: powerError, todayError: todayError, envError: envError, solarError: solarError }));
}
