...
export default function EnvContainer() {
    ...
    const [deviceId, setDeviceId] = useState(21); // 기본 21
    ...
    const recompute = (rows) => {
        const temps = rows.map((r) => r.temperature).filter((v) => v != null && Number.isFinite(v));
        const hums = rows.map((r) => r.humidity).filter((v) => v != null && Number.isFinite(v));
    ...
    return (_jsx(EnvPresenter, { mode: mode, setMode: setMode, preset: preset, setPreset: setPreset, onQuery: mode === 'realtime' ? pullOnce : queryRange, data: data, stats: stats, loading: loading, error: error, logs: logs, deviceId: deviceId, setDeviceId: setDeviceId }));
}
