 ...
 * EnvContainer.tsx
 *
 * 환경센서 데이터 관리 Container
 * - 줌 레벨, 시간 범위, 실시간 폴링 관리
 * - Solar와 동일한 구조
 */
...
import { fetchEnvData } from '@/api/env';
...
const DEVICE_OPTIONS = [21, 22, 23]; // Env 센서 ID
...
export const EnvContainer: React.FC = () => {
    ...
    const load = useCallback(
        async (opts?: { start?: string; end?: string }) => {
            ...
            try {
                const response = await fetchEnvData({
                    deviceid: deviceId,
    ...
    const statsTemp = useMemo(() => calcStats(data, 'temperature'), [data]);
    const statsHumidity = useMemo(() => calcStats(data, 'humidity'), [data]);
